...
### 4. layered-to-drawio.py

基于分层架构 JSON 配置生成可编辑的 **drawio XML**（`.drawio`）。可直接在 draw.io 桌面版、VS Code drawio 插件或 diagrams.net 中二次编辑。

**用法：**
```bash
python3 ./layered-to-drawio.py \
  --config ../fixtures/layered-architecture-example.json \
  --output ../output/my-arch.drawio
```

**JSON Schema（与 `layered-to-pptx.py` 共享）：**
```json
{
  "title":    "标题",
  "subtitle": "一句话定位（可选）",
  ...
  "footer":   "横向能力说明（可选）",
  "arrows":   ["L1↔L2 标签", "L2↔L3 标签"],
  "layers": [
    {
      "idx": "01", "name": "前端展示层", "en": "Presentation Layer",
      "highlights": "要点 A  ·  要点 B  ·  要点 C",
      "bg": "#eff6ff", "border": "#93c5fd", "accent": "#1d4ed8",
      "components": [ {"title": "X", "subtitle": "..."} ],
      "chips": ["Vue 3", "TypeScript"]
    }
  ]
}
```
...
/Applications/draw.io.app/Contents/MacOS/draw.io \
  -x -f svg --border 10 -o out.svg my-arch.drawio
/Applications/draw.io.app/Contents/MacOS/draw.io \
  -x -f png --border 10 --width 2400 -o out.png my-arch.drawio
```

### 5. layered-to-pptx.py

基于**同一份 JSON**生成可编辑的 **PowerPoint**（`.pptx`）。所有元素都是 PPT 原生形状（圆角矩形/连接线/文本框），不是嵌入图片，可在 PowerPoint / WPS / Keynote 中直接改字改色改位置。
...
**用法：**
```bash
python3 ./layered-to-pptx.py \
  --config ../fixtures/layered-architecture-example.json \
  --output ../output/my-arch.pptx
```

**输出特性：**
- 16:9 宽屏幻灯片（13.333 × 7.5 英寸）
- 一层一条彩色色带 + 编号条 + 亮点说明框 + 组件卡片 + 技术栈药丸
- 层间双向连接线（`<a:headEnd>` + `<a:tailEnd>`）并带标签
- 备注页自动写入每层的技术栈清单，便于演讲
- 与 `layered-to-drawio.py` 共用同一 JSON，**一源两图**

### 6. test-all-styles.sh
...
│   ├── layered-to-drawio.py   # JSON → 可编辑 drawio XML
│   ├── layered-to-pptx.py     # JSON → 可编辑 PowerPoint
...
- **v1.1.0** (2026-04-22) - 新增可编辑格式
  - `layered-to-drawio.py`：JSON → 可编辑 drawio XML
  - `layered-to-pptx.py`：JSON → 可编辑 PowerPoint（原生形状）
  - `fixtures/layered-architecture-example.json` 示例配置
  - 两脚本共用同一份 JSON Schema
