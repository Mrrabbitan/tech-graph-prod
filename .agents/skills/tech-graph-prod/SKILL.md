...
description: >-
  ...
  SVG+PNG, editable drawio XML, or editable PPTX. Trigger on: "画图" "帮我画"
  "生成图" "做个图" "架构图" "流程图" "可视化一下" "出图" "xml 格式" "drawio"
  "ppt" "powerpoint" "generate diagram" "draw diagram" "visualize" or any
  system/flow description the user wants illustrated.
---

# Fireworks Tech Graph

Generate production-quality technical diagrams in three interchangeable formats from the same source:

1. **SVG + PNG** via `rsvg-convert` — default, for blogs / docs / README images.
2. **drawio XML** (`.drawio`) — editable in draw.io desktop, VS Code drawio extension, or diagrams.net web.
3. **Editable PPTX** (`.pptx`) — every element is a native PowerPoint shape (no embedded raster), open in PowerPoint / WPS / Keynote.

All three share the same color palette and layout heuristics, so a single prompt can produce all three artifacts with consistent styling.
...
Do not pass `tech-graph-prod` directly to `skills add`, because the CLI expects a GitHub or local repository source.
...
### 5. `layered-to-drawio.py` - JSON → editable drawio XML
```bash
python3 ./scripts/layered-to-drawio.py \
    --config fixtures/layered-architecture-example.json \
    --output output/my-arch.drawio
```
- Consumes a layered-architecture JSON config (see `fixtures/layered-architecture-example.json`)
- Emits a `.drawio` (mxfile) XML with numbered index strip, component cards, tech-stack chips, and labeled bidirectional inter-layer connectors
- Convert to SVG/PNG afterwards via the draw.io CLI:
  ```bash
  /Applications/draw.io.app/Contents/MacOS/draw.io -x -f svg -o out.svg out.drawio
  /Applications/draw.io.app/Contents/MacOS/draw.io -x -f png --width 2400 -o out.png out.drawio
  ```
- Pure-Python, no dependencies beyond stdlib

### 6. `layered-to-pptx.py` - JSON → editable PowerPoint
```bash
...
```
- Consumes the **same** JSON config as `layered-to-drawio.py` — one source, two formats
- Every element is a native `MSO_SHAPE` (rounded rect / connector / text box); nothing is rasterized
- 16:9 widescreen slide (13.333 × 7.5 inches), editable fills/strokes/fonts
- Writes Speaker Notes containing a machine-readable layer summary (name + tech-stack chips)
- Opens cleanly in PowerPoint, WPS Office, and Keynote
...
- Use `layered-to-drawio.py` / `layered-to-pptx.py` whenever the user asks for "xml 格式 / drawio / ppt / 可编辑 / editable" — always prefer these over hand-writing mxfile or pptx XML
...
### A. Default SVG path (blogs, README, docs)
...
### B. Editable-artifact path (drawio XML / PPTX)

Use this path whenever the user says any of: "xml 格式" "drawio" "ppt" "powerpoint" "可编辑" "editable" or any time the deliverable must be editable downstream.

1. **Classify** as a **layered architecture** (the schema is optimized for 3–7 stacked layers with components + tech-stack chips). For non-layered diagrams, fall back to path A.
2. **Extract the 5 structural elements** from the user's description:
   - `title` / `subtitle`
   - N `layers`, each with: `idx`, `name`, `en`, `highlights`, `bg`/`border`/`accent` colors, 3–5 `components` (title + subtitle), and 4–7 `chips` (tech-stack items).
   - `arrows` (one label per inter-layer gap, bidirectional by default) — optional.
   - `footer` for cross-cutting concerns — optional.
3. **Choose the color palette** — reuse the default 5-layer palette unless user overrides:
   - L1 blue `#eff6ff / #93c5fd / #1d4ed8`
   - L2 indigo `#eef2ff / #a5b4fc / #4338ca`
   - L3 orange `#fff7ed / #fdba74 / #c2410c` (core — most emphasized)
   - L4 emerald `#ecfdf5 / #6ee7b7 / #047857`
   - L5 slate `#f8fafc / #94a3b8 / #334155`
4. **Write the JSON config** to a fixture file (or pass via stdin). Mirror the shape of `fixtures/layered-architecture-example.json`.
5. **Generate editable artifacts**:
   ```bash
   python3 scripts/layered-to-drawio.py -c config.json -o output/<name>.drawio
   python3 scripts/layered-to-pptx.py   -c config.json -o output/<name>.pptx
   ```
6. **(Optional) Render previews** for the user:
   ```bash
   /Applications/draw.io.app/Contents/MacOS/draw.io -x -f svg --border 10 \
       -o output/<name>.svg output/<name>.drawio
   /Applications/draw.io.app/Contents/MacOS/draw.io -x -f png --border 10 --width 2400 \
       -o output/<name>.png output/<name>.drawio
   ```
7. **Validate**:
   - drawio: `python3 -c "import xml.etree.ElementTree as ET; ET.parse('out.drawio')"`
   - pptx: `python3 -c "from pptx import Presentation; Presentation('out.pptx')"`
8. **Report** the generated file paths and note which can be edited in which tool.

## Layered Architecture (drawio + PPTX) — JSON Schema

When the user asks for a multi-layer technical architecture (typically 3–7 stacked bands, each with components + tech-stack), use the layered schema consumed by both `layered-to-drawio.py` and `layered-to-pptx.py`:

```json
{
  "title":    "My System — Tech Architecture",
  "subtitle": "One-line positioning statement",
  "page":     {"width": 1440, "height": 1050},
  "footer":   "Cross-cutting concerns line (optional)",
  "arrows":   ["Label between L1↔L2", "Label between L2↔L3", "..."],
  "layers": [
    {
      "idx":        "01",
      "name":       "Layer name (中文/English)",
      "en":         "English subtitle",
      ...
      "accent": "#1d4ed8",
      "components": [
        { "title": "Component A", "subtitle": "sub · detail · tag" },
        { "title": "Component B", "subtitle": "sub · detail · tag" }
      ],
      "chips": ["Framework", "Library", "Protocol", "DB"]
    }
  ]
}
```
...
**Schema guidance**:
- 3–7 layers fit best; the layout auto-divides the component row width across all components.
- Prefer **3–5 components per layer**; ≥6 will shrink cards but still work.
- Chip labels are short (≤12 CJK chars / ≤22 ASCII chars); widths are auto-estimated.
- `arrows.length` should be `layers.length - 1`. Missing entries render an arrow without a label.
- The default palette (blue → indigo → orange → emerald → slate) highlights L3 as the orchestration/core layer.
- For UML-style/flow diagrams, use path A (hand-written SVG) instead — the layered schema is intentionally rigid.
...
- **SVG + PNG** (default): `./[derived-name].svg` and `./[derived-name].png`
  - `rsvg-convert -w 1920 file.svg -o file.png` (1920px = 2x retina)
- **drawio XML** (editable): `./[derived-name].drawio`
  - Produced by `scripts/layered-to-drawio.py`
  - Re-render: `/Applications/draw.io.app/Contents/MacOS/draw.io -x -f svg -o out.svg out.drawio`
- **PPTX** (editable PowerPoint): `./[derived-name].pptx`
  - Produced by `scripts/layered-to-pptx.py`
  - Opens in PowerPoint / WPS / Keynote; every shape editable (rounded rects, connectors, text boxes)
- **Custom path**: user specifies with `--output /path/` or `输出到 /path/`
- **All three formats from one JSON**: keep a single fixture, run both `layered-to-*` scripts, and render the drawio to SVG/PNG via draw.io CLI for a consistent brand-coherent bundle.
